...
def get_pre_of_bad(s):
    ...
    if res == SAT:  
        ...
        for i in range(0, num_inputs):
            pipt = frames[Fk].solver.val(prime_lit(inputs[i]))
            # print(pipt)
            if pipt > 0:
        ...
        for i in range(0, num_latches):
            l_val = frames[Fk].solver.val(prime_lit(latches[i]))
            # print("l_val: ", l_val)
            if l_val > 0:
...
def encode_translation(s,satelite,cons = True):
    ...
    if satelite == None:
        ...
        for i in blif.inputs:
            satelite.freeze_var(i)
            satelite.freeze_var(prime_var(i))
        for l in blif.latches:
        ...
        for x in xors:
            assert x[0] > 0, f"Xor门输出x[0]必须为正数，实际为{x[0]}"
            if x[0] in lit_set:
                lit_set.add(abs(x[1]))
                lit_set.add(abs(x[2]))
                ...
                if x[0] in prime_lit_set:
                    po = prime_lit(x[0])
                    pi1 = prime_lit(x[1])
                    pi2 = prime_lit(x[2])
                    
                    prime_lit_set.add(abs(x[1]))
                    prime_lit_set.add(abs(x[2]))
...
def translate_to_dimacs():
    ...
    # 输入信号
    for i in range(num_inputs):
        variables.append(Variable(2 + i, None, 'i', i, 0))
        inputs.append(blif.inputs[i])

    # 锁存器（latch）
    for i in range(num_latches):
        variables.append(Variable(2 + num_inputs + i, None, 'l', i, 0))
        latches.append(blif.latches[i][1])

    # AND门
    for i in range(num_ands):
        o = 2 + num_inputs + num_latches + i
        in1 = blif.ands[i][1]
        in2 = blif.ands[i][2]
        # 直接使用BLIF信号编号
    ...
    for i in range(num_xors):
        o = 2 + num_inputs + num_latches + num_ands + i
        in1 = blif.xors[i][1]
        in2 = blif.xors[i][2]
